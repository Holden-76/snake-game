...
function loadHighScore() {
  const stored = Number.parseInt(localStorage.getItem("slither_high_score"), 10);
...
function updateHighScore() {
  if (score > highScore) {
    highScore = score;
    localStorage.setItem("slither_high_score", String(highScore));
    updateHighScoreLabel();
  }
}
...
function handleDirectionChange(event) {
  ...
  const currentHeading = queuedDirection;
  const isOpposite = currentHeading.x + next.x === 0 && currentHeading.y + next.y === 0;
  if (!isOpposite) {
